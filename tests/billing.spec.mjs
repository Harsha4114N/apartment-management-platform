 ...
 *      → verify "All Flats" badge in expenses table
 *   2. Data Isolation & Math — ResidentA sees ₹1000, ResidentB sees ₹1000
 *      → proves 2000/2=1000 AND each only sees own flat's bills
 *   3. Targeted Splitting — Admin creates ₹500 expense targeted to Flat101 only
 *      → ResidentA gets ₹500 (total 2 bills), ResidentB unchanged (1 bill of ₹1000)
 */
 ...
 * the role-specific dashboard to load.  This guarantees the auth token
 * is stored in localStorage before any subsequent navigation.
 */
async function loginAs(page, email, password, expectedUrlPattern) {
  ...
  // Wait for React Router to land on the role-specific dashboard
...
test.describe('Financial Engine E2E Tests', () => {
  ...
  test('Scenario 1: Split Equally Across ALL Flats', async ({ browser }) => {
    ...
    // Navigate to the billing page (/bills works for both Admin and Resident)
  ...
  //  ResidentA sees 1 bill of ₹1000, ResidentB sees 1 bill of ₹1000
  // ═══════════════════════════════════════════════════════════════════
  test('Scenario 2: Data Isolation & Math Verification', async ({ browser }) => {
    ...
    // Wait for bills to load (the ResidentView shows "Loading your bills..." first)
    await pageA.waitForSelector('h2:has-text("Pending Bills")', { timeout: 15000 });

    // Allow React to finish rendering all bill card elements
    await pageA.waitForTimeout(2000);

    // Check that the pending count badge shows "1"
    ...
    // Verify the bill amount is ₹1000
    await expect(pageA.locator('text=₹1000').first()).toBeVisible();

    // Verify the "Pay ₹1000" button exists
  ...
  // ═══════════════════════════════════════════════════════════════════
  //  SCENARIO 3: Targeted Splitting — TARGET Flat101 for ₹500
  //  ResidentA gets new ₹500 bill, ResidentB unchanged at 1 bill ₹1000
  // ═══════════════════════════════════════════════════════════════════
  test('Scenario 3: Targeted Splitting', async ({ browser }) => {
    ...
    // Select TARGET radio — click the label to ensure React state updates
    await adminPage.click('label:has-text("Target specific flats")');
    // Allow React to render the conditional input
    await adminPage.waitForTimeout(500);

    // Fill in target flat numbers (the input appears conditionally when TARGET is selected)
    ...
    // Pending count badge should show "2"
    const pendingBadgeA = pageA.locator('h2:has-text("Pending Bills") span').first();
    await expect(pendingBadgeA).toHaveText('2');

    // Should see both ₹1000 and ₹500 amounts
    await expect(pageA.locator('text=₹1000').first()).toBeVisible();
    await expect(pageA.locator('text=₹500').first()).toBeVisible();

    // Should have 2 Pay buttons
    await expect(pageA.locator('button:has-text("Pay ₹1000")').first()).toBeVisible();
    ...
    // ── 3c. Resident B — should still have 1 bill of ₹1000 (untouched) ──
    const errorsB = [];
    const contextB = await browser.newContext();
    const pageB = await contextB.newPage();
    attachErrorListeners(pageB, errorsB);
    ...
    // Still shows 1 pending bill
    const pendingBadgeB = pageB.locator('h2:has-text("Pending Bills") span').first();
    await expect(pendingBadgeB).toHaveText('1');

    // Still ₹1000 — no ₹500 bill
    await expect(pageB.locator('text=₹1000').first()).toBeVisible();
    await expect(pageB.locator('button:has-text("Pay ₹1000")').first()).toBeVisible();

    // Verify ₹500 does NOT appear for Resident B
